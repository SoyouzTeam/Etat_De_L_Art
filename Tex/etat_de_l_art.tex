\documentclass[12pt,a4paper,openany]{report}
\usepackage[utf8]{inputenc}
\usepackage{ucs}
\usepackage[T1]{fontenc}
\usepackage{graphicx}
\usepackage[francais]{babel} 
\usepackage{array} 
\usepackage{color}
\usepackage{url}
\usepackage{natbib}
\bibliographystyle{plain}
\usepackage[pdfborder=0]{hyperref}%pour faire des ancres Table des matières -> partie A | partie B | etc
\definecolor{blanc}{rgb}{0.0,0.0,0.0}
\hypersetup{
    hyperfigures = true,
    colorlinks = true,
    linkcolor=blanc,
    citecolor=blue,
    urlcolor=blue
   }

%%%%%%%%%%%%% MARGES %%%%%%%%%%%%%%%%%%
% Package pour modifier les MARGES !
\usepackage{geometry} 
\geometry{ hmargin=3cm, vmargin=3cm } % hmargin > marges de coté | vmargin > marges haut/bas
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\setcounter{secnumdepth}{4}
\setcounter{tocdepth}{3}
\makeatletter
\newcounter {subsubsubsection}[subsubsection]
\renewcommand\thesubsubsubsection{\thesubsubsection .\@alph\c@subsubsubsection}
\newcommand\subsubsubsection{\@startsection{subsubsubsection}{4}{\z@}%
                                     {-3.25ex\@plus -1ex \@minus -.2ex}%
                                     {1.5ex \@plus .2ex}%
                                     {\normalfont\normalsize\bfseries}}
\newcommand*\l@subsubsubsection{\@dottedtocline{3}{10.0em}{4.1em}}
\newcommand*{\subsubsubsectionmark}[1]{}
\makeatother


\title{Etat de l'art}
\author{Projet DUT INFO}
\date{17 Septembre 2013}

\begin{document}

\maketitle

\hypertarget{tableofcontents}{} %affecte des ancres aux differentes partie
\tableofcontents

\part{Introduction}

\chapter{Fonctionnement de l'affichage sur ordinateur}
\input{gpu_et_cpu/histoire}

\part{Pipeline graphique}
\chapter{Fonctionnement du pipeline graphique}
\input{pipeline/pipelinegraphique}

\chapter{La composante lumière}
\input{pipeline/lumiere}

\chapter{Shader}
\input{pipeline/shader}

\part{Outils de rendu 3D}

\chapter{OpenGL}

\input{openGL/openGl}%Include du fichier openGl.tex

\chapter{Direct3D}
\input{directx/directx}%Include du fichier matlab.tex

\chapter{MatLab}
\input{matlab/matlab}%Include du fichier matlab.tex

\input{annexes/annexes}
\newpage
\bibliography{bibTex/biblio}

\end{document}